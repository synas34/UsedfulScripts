...
#NOTES: improvement over v2: processing departure time inside the loop to
#account for different departure times per observation
warning("This client code is for variable departure times (one per observation).
         For fixed departure times please use v2")
...
LOS.google_vdt<-function(origin,destination,
                                     mode="driving",
                                     avoid="",traffic_model="",
  ...
  #-processed inside the loop below
  ...
  for (i in 1:length(origin)){
    #processing departure time inside the loop to account for different departure
    #times per observation
    if(mode=="walking" | departure_date[i]=="" | departure_time[i]==""){
      dep_time=""
    }else{
      dep_time<-paste(departure_date[i],departure_time[i])
